...
def generate_launch_description():
    ...
    dbc_file_path = get_share_file(
        package_name='raptor_dbw_can', file_name='launch/CAN1-INDY-V14.dbc'
    )
    dbc_file_path2 = get_share_file(
        package_name='raptor_dbw_can', file_name='launch/CAN2-INDY-V0.dbc'
    )
    dbc_file_path3 = get_share_file(
        package_name='raptor_dbw_can', file_name='launch/CAN3-INDY-V0.dbc'
    )
    ...
    ssc_interface = Node(
        ...
        remappings=[
            ...
            ('ct_status2', '/raptor_dbw_interface/ct_report2'),

            ('/vehicle/misc_report', '/raptor_dbw_interface/misc_report_do'),
            ('/vehicle/pt_report', '/raptor_dbw_interface/pt_report'),
            ('/vehicle/accelerator_report',
             '/raptor_dbw_interface/accelerator_report'),
            ('/vehicle/brake_pressure_report',
    ...
    socketcan1_receiver_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            get_share_file(
                package_name='raptor_dbw_can',
                file_name='launch/can2_receiver.launch.py'
            )
        ]),
        launch_arguments={'interface': 'can1'}.items()
    )

    socketcan1_sender_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            get_share_file(
                package_name='raptor_dbw_can',
                file_name='launch/can2_sender.launch.py'
            )
        ]),
        launch_arguments={'interface': 'can1'}.items()
    )

    socketcan2_receiver_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            get_share_file(
                package_name='raptor_dbw_can',
                file_name='launch/can3_receiver.launch.py'
            )
        ]),
        launch_arguments={'interface': 'can2'}.items()
    )

    socketcan2_sender_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            get_share_file(
                package_name='raptor_dbw_can',
                file_name='launch/can3_sender.launch.py'
            )
        ]),
        launch_arguments={'interface': 'can2'}.items()
    )
        ...
        parameters=[
            {
                ...
                'dbw_dbc_file2': dbc_file_path2,
                'dbw_dbc_file3': dbc_file_path3,
                'veh_number': 4
            }
        ],
        remappings=[
            ...
            ('/raptor_dbw_interface/can1_rx', '/can1/from_can_bus'),
            ('/raptor_dbw_interface/can1_tx', '/can1/to_can_bus'),
            ('/raptor_dbw_interface/can2_rx', '/can2/from_can_bus'),
            ('/raptor_dbw_interface/can2_tx', '/can2/to_can_bus'),
        ],
    )
    ...
    nif_localization_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            get_package_share_directory(
                'nif_localization_nodes') + '/launch/deploy.launch.py'
        ),
    )
    ...
    # Map based Localization, CPU Overload Issue(disable) 
    # nif_resilient_localization_launch = IncludeLaunchDescription(
    #     PythonLaunchDescriptionSource(
    #         get_package_share_directory(
    #             'nif_resilient_localization_nodes') + '/launch/deploy.launch.py'
    #     ),
    # )
...
# NIF CONTROL STACK ###############################################
    ...
    nif_csl_node = Node(
        ...
        remappings=[
            ...
            ('out_desired_acceleration_cmd',
             '/control_safety_layer/out/desired_accel'),
        ]
    )
    ...
    pi_controller_config_file = get_share_file(
        package_name='nif_control_joint_lqr_nodes', file_name='config/pi_controller_config.yaml'
    )
    ...
    nif_joint_lqr_control_node = Node(
        ...
        parameters=[
            LaunchConfiguration('control_joint_lqr_params_file'),
            {
                ...
                'pi_config_file': pi_controller_config_file,
        ...
        remappings=[
            ...
            ('in_reference_path', '/Emergency/final_path'),
            ('in_reference_trajectory', '/planning/dynamic/traj_global'),
            ('/oppo_is_back', '/opponent_prediction/oppo_is_back'),
        ]
    )
    ...
    nif_velocity_planning_node = Node(
        ...
        remappings=[
            ...
            ('in_imu_data', 'novatel_bottom/rawimux'),
            # ('in_inspva', 'novatel_bottom/inspva'),  #Uncomment to activate dynamic roll
        ...
        parameters=[{
            ...
            'max_ddes_vel_dt_green_flag': 5.0,
...
# NIF LQR + CSL END ###############################################
    ...
    if track == LOR:
        ...
        global_params_file = 'params_TMS.global.yaml'
    elif track == MONZA:
        global_params_file = 'params_MONZA.global.yaml'
    elif track == MONZA_SIM:
        global_params_file = 'params_MONZA_SIM.global.yaml'
    else:
    ...
    robot_description_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            get_package_share_directory(
                'av21_description') + '/launch/deploy.launch.py'
        )
    )
...
### NIF WAYPOINT MANAGER #############################
    ...
    if track == LOR:
        ...
        config_file = wpt_config_file_monza
    elif track == MONZA_SIM:
        config_file = wpt_config_file_monza_sim
    else:
        raise RuntimeError("ERROR: invalid track provided: {}".format(track))
...
### NIF WAYPOINT MANAGER END #############################
    ...
    nif_mission_manager_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            get_share_file("nif_mission_manager_nodes",
    ...
    nif_patchwork_plusplus = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            get_share_file("patchworkpp",
                           'launch/demo.launch.py')
        )
    )

    nif_polar_inverse = Node(
        package='nif_polar_inverse_nodes',
        executable='nif_polar_inverse_nodes_exe',
        output='screen',
        # parameters=[
        #     LaunchConfiguration('nif_adaptive_cruise_control_param'),
        #     {
    ...
    nif_costmap_generator = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            get_share_file("nif_costmap_generator",
                           'launch/deploy.launch.py')
        )
    )
    ...
    nif_waypoint_viz_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            get_share_file("waypoint_viz", 'launch/deploy.launch.py')
        )
    )

    nif_wall_detection_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            get_share_file("nif_points_preprocessor_nodes",
                           'launch/deploy.launch.py')
        )
    )
    fm_data_node = Node(
        package='fm_data',
        executable='fm_data_node',
        output='screen'
    )
...
### NIF MULTILAYER PLANNER END #############################
    ...
    return LaunchDescription([
        ...
        # nif_resilient_localization_launch,
        ...
        nif_waypoint_viz_launch,
        nif_wall_detection_launch,

        fm_data_node,
        socketcan1_receiver_launch,
        socketcan1_sender_launch,
        socketcan2_receiver_launch,
        socketcan2_sender_launch
