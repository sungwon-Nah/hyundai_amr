...
#include "nif_msgs/msg/planner_status.hpp"
#include "nif_msgs/msg/opponent_refline.hpp"
...
class DynamicPlannerNode : public nif::common::IBaseNode {
  ...
  enum OVERTAKE_STATUS { BEFORE, ATTEMPT, PASSING ,DONE};
public:
  ...
  void opponentReferenceCallback(
  ...
  stitchFrenetToPath(std::shared_ptr<FrenetPath> &frenet_segment_,
                     nav_msgs::msg::Path &target_path_);
  nif_msgs::msg::DynamicTrajectory
  findOvertakingPath(double default_planning_time_min, 
                    double allowable_maximum_vy, bool for_overtake);
private:
  ...
  rclcpp::Subscription<nif_msgs::msg::DynamicTrajectory>::SharedPtr
      m_oppo_pred_sub;
  // NEW: opponent reference line prediction reesult
  rclcpp::Subscription<nif_msgs::msg::OpponentRefline>::SharedPtr
        m_oppo_refline_sub;
  ...
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubPathCandidatePoints;

  rclcpp::Publisher<nif_msgs::msg::PlannerStatus>::SharedPtr m_planner_status_pub;
  // output timer
  ...
  nif_msgs::msg::OpponentRefline m_opponent_reference_line = nif_msgs::msg::OpponentRefline();
...
//////////////
  // PIT LINE
  //////////////
  std::string m_pitline_file_path;
  std::vector<double> m_pitline_x_vec, m_pitline_y_vec;
  nav_msgs::msg::Path m_pitline_path;
  nif_msgs::msg::DynamicTrajectory m_pitline_dtraj;
  FrenetPathGenerator::CubicSpliner2DResult m_pitline_spline_data;
  double m_pitline_full_progress;
  ...
  // CENTER LINE
  ////////////////
  std::string m_center_file_path;
  std::vector<double> m_center_x_vec, m_center_y_vec;
  nav_msgs::msg::Path m_center_path;
  nif_msgs::msg::DynamicTrajectory m_center_dtraj;
  FrenetPathGenerator::CubicSpliner2DResult m_center_spline_data;
  double m_center_full_progress;

  bool m_defender_mode_first_callback = false;
  bool m_race_mode_first_callback = false;
  bool m_stop_mode_first_callback = true;
  bool m_non_overtaking_mode_first_callback = false;
  bool m_pit_mode_first_callback = false;
...
//   const double m_acceptable_slip_angle_rad = 0.0872665; // 5 deg
  const double m_acceptable_slip_angle_rad = 0.0698132; // 4 deg
  const double m_merging_back_gap_thres = 20.0;         // meter
  ...
  string m_last_update_target_path_alias = m_pitline_file_path;
  LATERAL_PLANNING_TYPE m_last_lat_planning_type;
  LONGITUDINAL_PLANNING_TYPE m_last_long_planning_type;

  pcl::PointCloud<pcl::PointXYZI>::Ptr path_candidates;
  int m_overtake_decision_counter = 0; // 50Hz timer callback
  int m_overtake_status = OVERTAKE_STATUS::BEFORE;
  unsigned int m_backstretch_zone[2] = {0, 0};
};
